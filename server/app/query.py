...
#     # This function is used to process the user query and generate course content by using information from the vector database
...
#         retrieved_topic = rag_retrieval(query)
#         content = query_llm(retrieved_topic)
#         topics_dict[topic] = content
...
def fetch_topics(query):
    ...
    A user has provided a chapter topic, and your task is to break it down into three of its core subtopics.
...
def rag_retrieval(topic):
    ...
    final_prompt = PROMPT_TEMPLATE.format(context=context_text, topic=topic)
